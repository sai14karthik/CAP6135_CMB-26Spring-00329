...
Specifically, this study explores the integration of Recurrent Neural Networks (RNNs) to provide the temporal "memory" necessary to detect multi-stage attacks. By training these models on standardized benchmarks such as the CICIDS-2017 and NSL-KDD datasets, we aim to establish a system that does not ask, "Does this file match a known threat?" but rather, "Does this behavior deviate from the statistical baseline of a healthy network?" \cite{sharafaldin2018}. The goal of this project is to evaluate the performance of these architectures in terms of accuracy, precision, recall, $F_1$ score, and false positive rate, ultimately providing a blueprint for an automated, proactive cybersecurity posture that can be checked against the accompanying PyTorch implementation.
...
\subsection{Reference Implementation and Repository}
\label{sec:refimpl}
The architectural and preprocessing choices above are mirrored in a PyTorch reference implementation (see \texttt{intrusion\_detection\_dl/} in the course GitHub repository \texttt{github.com/sai14karthik/CAP6135\_CMB-26Spring-00329}). Concretely, the standalone DNN uses three fully connected layers with widths $128\!\rightarrow\!64\!\rightarrow\!32$, ReLU activations, dropout probability $0.2$ after each hidden layer, and a two-way softmax readout. The recurrent model reported as ``RNN'' in our tables is instantiated as a \emph{single-layer LSTM} with hidden dimension $64$, consistent with Section~III's LSTM formulation. The hybrid model applies the \emph{same} $128\!\rightarrow\!64\!\rightarrow\!32$ dense stack independently at each sequence timestep, followed by the $64$-dimensional LSTM and softmax head---this matches the spatial--temporal stacking described conceptually earlier. Training minimizes weighted cross-entropy (inverse class frequency weights) with the Adam optimizer at learning rate $10^{-3}$ by default. Evaluation code computes accuracy, precision, recall, $F_1$, and FPR from the confusion matrix using the same formulas as in Section~III. For CSV exports of NSL-KDD or CICIDS-2017, the loader performs one-hot encoding of remaining categorical fields where applicable and min--max scaling to $[0,1]$; a synthetic sequence generator is provided for smoke testing when raw captures are not mounted locally.

\section{Results and Performance Analysis}
We report comparative behavior for three models trained and scored under the same preprocessing pipeline and metric definitions from Section~III (and Section~\ref{sec:refimpl}): a standalone DNN, a standalone RNN (implemented as an LSTM), and a hybrid DNN--LSTM stack. Aggregate metrics quantify how often the system is correct overall (accuracy), how trustworthy positive alarms are (precision), how completely malicious activity is captured (recall), and how those objectives trade off in a single scalar ($F_1$). Because rare, staged intrusions are often drowned out by volumetric noise, we additionally examine per-class recall on NSL-KDD and on a focused subset of CICIDS-2017 labels where class imbalance is most acute.

\subsection{Standardized Metric Performance}
At the corpus level, all three architectures achieve strong separation between benign and attack traffic, yet the ordering is stable: the recurrent model improves on the dense baseline, and the hybrid configuration yields the strongest joint gains on both benchmarks. CICIDS-2017 is the more demanding setting, with modern denial-of-service, bot-assisted, and credential-focused patterns; the hybrid model's lead over the DNN is therefore slightly larger there than on NSL-KDD, which aligns with the hypothesis that temporal structure matters once attacks depart from simple volume spikes. Absolute percentages are sensitive to train--test splits and random seeds; Section~\ref{sec:refimpl} documents the released code so that architectures and metrics can be re-audited even when point estimates shift slightly.
